...
import { CloudUser } from 'src/app/shared/model/cloud-user.model';
...
export class AuthService {
  ...
  private clientId = 'pS4x0OxdjNehijYrqLB26MuQWXETCUO4';
  cloudUser?: CloudUser;
  token?: string;

  auth0Client$ = (from(
    createAuth0Client({
      domain: 'dev--cloud-storage.eu.auth0.com',
      client_id: this.clientId,
      audience: 'storage.cloudstorage.com',
  ...
  handleAuthCallback() {
    ...
    authComplete$.subscribe(([user, token, loggedIn]) => {
      ...
      this.cloudUser = {
        ...
        sub: authUser.sub!.substr(6, authUser.sub?.length)
      };
